...
if [ "$1" = "--skip-docker" ]; then
    ...
    docker compose run --rm web odoo -c /etc/odoo/odoo.conf -d elgordo \
      -i pos_hr,pos_kitchen_screen_odoo,pos_receipt_logo,pos_kitchen_receipt,pos_takeaway --stop-after-init
...
docker compose run --rm web odoo server -c /etc/odoo/odoo.conf -d elgordo \
  -i base,stock,mrp,point_of_sale,pos_restaurant,pos_hr,pos_kitchen_screen_odoo,pos_receipt_logo,pos_kitchen_receipt,pos_takeaway --stop-after-init
