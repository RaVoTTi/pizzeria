...
class PosOrder(models.Model):
    ...
    def get_details(self, shop_id, *args, **kwargs):
        ...
        for order in pos_orders:
            has_kitchen_items = any(
                categ.id in category_ids
                for line in order.lines
                for categ in line.product_id.pos_categ_ids
            )
            if has_kitchen_items:
                kitchen_orders |= order
        pos_lines = kitchen_orders.lines.filtered(
            lambda line: any(
                categ.id in category_ids
                for categ in line.product_id.pos_categ_ids
            )
        )
        values = {"orders": kitchen_orders.read(), "order_lines": pos_lines.read()}
    ...
    def action_pos_order_paid(self):
        ...
        When an order is paid, we mark it for the kitchen screen but preserve
        its current workflow status (draft/waiting).  The kitchen must still
        explicitly complete the order via the Accept/Done flow — paying does
        NOT mean the food is already served.
        """
